...
use diesel::{BoolExpressionMethods, ExpressionMethods, Insertable, QueryDsl, QueryResult, RunQueryDsl, SqliteConnection, Table};
use diesel::connection::SimpleConnection;
use diesel::r2d2::{ConnectionManager, Error};
use r2d2::{Pool, PooledConnection};
use crate::models::{Masternode, MasternodeList, NewMasternode, NewMasternodeList, NewQuorum, Quorum};
...
pub fn get_connection_pool() -> Pool<ConnectionManager<SqliteConnection>> {
    Pool::builder()
        .max_size(1)
        .connection_customizer(Box::new(ConnectionOptions { enable_wal: true, enable_foreign_keys: true, busy_timeout: Some(Duration::from_secs(30)) }))
        .build(ConnectionManager::<SqliteConnection>::new("DashSyncRS.sqlite"))
        .unwrap()
}

fn get_pooled_connection() -> PooledConnection<ConnectionManager<SqliteConnection>> {
